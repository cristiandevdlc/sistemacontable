...
const ServiciosTransito = () => {
	...
	const fetchdata = async () => {
		const response = await request(route("servicios-en-transito", {estatus:[1, 4], type: 1}), "GET");
	...
	const submit = async () => {
		...
		if (state.action === 'cancelar') {
			await request(
				route('pedidos-detalle.update', data.detalles.pedidoDetallesId),
				"PUT",
				{ ...data, estatus: 3, detalles: { ...data.detalles, estatus: 3, servicio: data.detalles.servicio + ' ' + state.obs + ';' } })
				.then(() => {
					handleCloseModal()
					fetchdata()
				})
	...
	const envioFunc = async () => {
		if (data.detalles.enviado.toString() === "1") {
			...
			await request(
				route('pedidos-detalle.update', data.detalles.pedidoDetallesId),
				"PUT",
				{ ...data, detalles: { ...data.detalles, enviado: 1, servicio: data.detalles.servicio + ' ' + state.obs + ';' } })
				.then(() => {
					fetchdata()
				})
		}
				...
				<div className='flex gap-6 pt-6'>
					...
					{data.pedidoId &&
						<ServiciosTransitoDialog
							...
							pedido={data}
							check={state.check}
							onChangeVendedor={(value) => setData({ ...data, detalles: {...data.detalles, otroVendedor: value} })}
							onChangeCheck={(e) => {
...
export default ServiciosTransito