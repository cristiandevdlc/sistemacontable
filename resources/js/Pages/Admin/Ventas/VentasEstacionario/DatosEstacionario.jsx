...
import { IntRemissionTotals, remissionRules } from './IntVentaEstacionario';
...
export default function DatosEstacionario(data, empresa, states, totales, setTotales, remisionsData, setRemisionsData, setErrors) {
    ...
    const fetchData = async () => {
        const [
            ...
            request(route("productos-historico.remision"), 'POST', { zona: data.zona, tipoServicio: 'ESTACIONARIO' }, { enabled: true }),
            ...
            request(route('remisiones.unidad'), 'POST', { ...data, tipoServicio: ServiceTypes.ESTACIONARIO }, { enabled: true }),
        ]);
    ...
    const remissionsActions = (action) => {
        if (empresa.empresa_ConsultaRemision == NO_CONSULTA_REMISION || data.rutaTlmk == NO_CONSULTA_REMISION) {
            ...
            const remissionTotal = [...remisionsData.filter(r => !remisPedidos.includes(r.detalles?.remision))].reduce((prev, next) => {
                prev.lts = {
                    contado: next.tipoVenta == saleType.contado ? (parseFloat(prev.lts.contado ?? 0) + parseFloat(next.detalles?.Cantidad)) : parseFloat(prev.lts.contado),
                    credito: next.tipoVenta == saleType.credito ? (parseFloat(prev.lts.credito ?? 0) + parseFloat(next.detalles?.Cantidad)) : parseFloat(prev.lts.credito),
                    total: parseFloat(prev.lts.contado) + parseFloat(prev.lts.credito) + parseFloat(next.detalles?.Cantidad)
                }
                prev.price = {
    ...
    const calculateDiscount = (cantidad = 1, cliente = null, historico = null) => {
        ...
        if (cantidad && cliente && historico) {
            if (cliente.cliente_cienLitros == 0) {
                prices.descuento = cliente.cliente_descuento
                prices.total = getTotalFromCurrent(historico.historico_precio ?? 0, cantidad ?? 1, prices.descuento)
            }
            else {
                prices.descuento = (parseFloat(cantidad) < 100) ? 0 : cliente.cliente_descuento
                prices.total = getTotalFromCurrent(historico.historico_precio ?? 0, cantidad ?? 1, prices.descuento)
            }
        }
    ...
    const getTotalFromCurrent = (precio = 0, cantidad = 1, bono = 0) => ((precio - bono) * cantidad)
    ...
    return [
        ...
        },
        {
            ...
            customItem: ({ label }) => (<>
                <div style={{ zoom: 0.6 }}>
                    <Datatable
                        title="dasdasd"
                        data={remisionsTable.sort((a, b) => a.remision - b.remision)}
                        selection='multiple'
                        virtual={true}
                        searcher={false}
        ...
        },
        {
            ...
            onChangeFunc: (e, o) => {
                ...
                const { descuento, total } = calculateDiscount(currentRemision.detalles?.Cantidad, currentRemision.direccion?.cliente_facturacion, o.historico);
        ...
        },
        {
            ...
            type: "decimal",
            style: 'col-span-4 lg:col-span-2',
            value: String(parseFloat(currentRemision.total ?? 0) || 0),
            disabled: true,
            customIcon: "attach_money"
        },
        {
