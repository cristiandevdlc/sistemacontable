import { destinoData, destinoValidations } from './IntDestino'
...
const Destino = () => {
    const { data, setData } = useForm(destinoData)
    ...
    const fetchDestinos = async () => {
        const response = await request(route('transporte-destino.index'))
    ...
    useEffect(() => {
        if (destinos && localidades) setLoading(false)
    ...
    const submit = async () => {
        setErrors({})
        const result = validateInputs(destinoValidations, data)
        if (!result.isValid) {
        ...
        const url = action === "create" ? route('transporte-destino.store') : route('transporte-destino.update', data.idDestino);
    ...
    const coloniasPorCodigoPostal = async () => {
        const response = await request(route("colonia-por-cp", data.CodigoPostal), 'GET', {}, { enabled: true, error: { message: 'No existen datos enlazados a este C.P.', type: 'error' } });
        const finalResponse = await formatearRespuestColonias(response)
        setInfoByPostalCode(finalResponse)
        setData({
            ...data,
            idMunicipio: finalResponse.municipio?.idMunicipio,
            idEstado: finalResponse.estado?.idEstado,
            idPais: finalResponse.pais?.idPais,
            descripcionEstado: finalResponse.estado?.descripcionEstado,
            descripcionMunicipio: finalResponse.municipio?.descripcionMunicipio,
            descripcionPais: finalResponse.pais?.descripcionPais
        })
    }
            ...
            {!loading &&
                <div className='sm:h-[97%] md:h-[90%]'>
                    <Datatable
                        ...
                        virtual={true}
                        columns={[
                            { width: '15%', header: 'Nombre', accessor: 'NombreRFCDestino' },
                            { width: '15%', header: 'RFC', accessor: 'RFCDestino' },
                            {
                        ...
                        fields={[
                            {
                                ...
                                type: 'text',
                                style: 'col-span-2',
                                fieldKey: 'NombreRFCDestino',
                                value: data.NombreRFCDestino || '',
                                onChangeFunc: (e) => { setData({ ...data, NombreRFCDestino: e.target.value }) }
                            },
                            {
                                label: "RFC",
                                input: true,
                                ...
                                fieldKey: 'RFCDestino',
                                style: 'col-span-2',
                                value: data.RFCDestino || '',
                                onChangeFunc: (e) => {
                                    const inputValue = e.target.value;
                                    if (inputValue.length <= 13) {
                                        setData({ ...data, RFCDestino: inputValue });
                                    }
                                }
                            },
                            {
                            ...
                            },
                            {
                                ...
                                _conditional: (field) => {
                                    console.log(field)
                                    if (field.options.length === 0 && action === 'edit' && field.value)
                                        return false
                                    return true
                                },
